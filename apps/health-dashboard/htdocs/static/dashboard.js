...
(function () {
  ...
  function setupChartDragSelection(canvas, granularity, labels) {
    ...
    function onDocumentMouseUp(e) {
      ...
      const error = DataUtils.validateSpan(granularity, startDate, endDate, config);
  ...
  async function loadSeries() {
    try {
      ...
      WeightUtils.convertWeightPayload(payload, unitPreference, roundMetricValue);
      if (payload.unit === 'kilograms') {
        displayUnit = 'kilograms';
      } else if (payload.metric === 'weight') {
        payload.datasets.forEach(dataset => {
